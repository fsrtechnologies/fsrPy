...
class CodeFinder(object):
    ...
    def lookup(self, codeType, Code, wordwrapLen=80, includeNote=False):
        ...
        if not includeNote:
            idx = Message.find('Note:')
            if idx > 0:
                Message = Message[:idx]
            idx = Message.find('(Use ')
            if idx > 0:
                Message = Message[:idx]
            idx = Message.find(': ')
            if idx > 0:
    ...
    def loadTable(self):
        ...
        tree = etree.parse('E:\\Dropbox\\HIPAA\\5160000_Entity_Identifier_Code.xml')
        for pV in tree.iter(tag="permissibleValue"):
            startDate = pV.findtext('beginDate').split('T')[0]
            startDate = tmpDtTm.strptime(startDate, "%Y-%m-%d")
            stopDate = pV.findtext('endDate').split('T')[0]
            stopDate = tmpDtTm.strptime(stopDate, "%Y-%m-%d")
            modDate = startDate
            tmp = pV.find('value')
            code = tmp.findtext('code')
            tmp = pV.find('valueMeaning')
            desc = tmp.findtext('domainMeaningName')
            tmpCode = ResponseCode('Entity', code, desc, startDate, stopDate, modDate, '')
            self.session.add(tmpCode)
            codeCount +=1
...
if __name__ == '__main__':
    with CodeFinder(dbFile=r"C:\Users\Marc\Desktop\codeTable.db") as cf:
        cf.loadTable()
        codes = [('CARC','59'),('HPCC','12'),('FAKE','99'), ('Entity','PR')]
