...
class Global(object):
    ...
    MediMedi = boolean(default=False)
    MedicareCarrierCode = string(default="01192")
    """
    # Note - Hard-coding Medicare carrier here; for list, see Carrier.py
...
class Gem(object):
    def __init__(self, gemFile='2015_I9gem.txt'):
        self.Notes = {}
        self.Dir = {}
        with open(gemFile, 'r') as gF:
            for line in gF.readlines():
                tmpList = line.split()
                self.Dir[tmpList[0]] = [ tmpList[1], tmpList[2] ]

    def lookUp(self, diag):
        try:
            ICD10, gemNote = self.Dir[diag][0], self.Dir[diag][1]
            if gemNote == '00000':
                tmpNote = ' maps exactly to ICD-10 code {}'.format(ICD10)
            elif gemNote[0] == '1':
                tmpNote = ' is an approximate match to ICD-10 code {}'.format(ICD10)
            elif gemNote[1] == '1':
                tmpNote = ' cannot be mapped to ICD-10 at all.  Passing through as-is; likely rejection.'
                ICD10 = diag
            self.Notes[diag] = tmpNote
            return ICD10
        except KeyError:
            if len(diag) > 1:
                self.Notes[diag] = 'No record found in GEM file. Passing through as-is.'
            return diag
...
class Form1500(object):
    def __init__(self, TestPrint=False, leftBlock=0, midBlock=24, rightBlock=49, labelEdge=49, formVer="08-05"):
        ...
        self.Box30                = fldDef(Global.cfgFile['bodyRightBlock']+19,55, 7)
...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title, size, style = wx.DEFAULT_FRAME_STYLE ):
        ...
        self.gem = Gem(Global.cfgFile['gemFile'])
        ...
        fpOpt = {
            "UCF":{"Title":"Select the file containing UCF claims",   "Mask":"*.*",   "Mode":wx.FD_OPEN},
            "FI" :{"Title":"Select the form-image file ",             "Mask":"FI*.*", "Mode":wx.FD_OPEN},
            "Out":{"Title":"Where should we save the converted file?","Mask":"*.*",   "Mode":wx.FD_SAVE}, }
        self.filePicker ={}
        ...
        Opts = [
            ...
            {"Name": "ICD10", "CtrlType": "CheckBox", "Label":"Print diagnoses as ICD-10?", "Tip":"Should we treat diagnosis codes as ICD-9 or -10?"},
            {"Name": "MediMedi", "CtrlType": "CheckBox", "Label":"Medi-Medi?", "Tip":"Should we do special processing for Medi-Cal secondary billing?\n  CAREFUL! Don't check this for regular billing!"},
    ...
    def ParseOld1500(self, pathStr):
        ...
        def fld2_4():
            Global.Claims[Global.claimCount].Box2.Value =  inLine[:28].strip().replace('  ', ', ') #Insert comma after last name if none exists
        ...
        def fld9_11():
            Global.Claims[Global.claimCount].Box9Name.Value =  inLine[:30].strip().replace('  ', ', ') #Insert comma after last name if none exists
        ...
        def fld9c_11c():
            ...
            if Global.cfgFile['MediMedi']:
                Global.Claims[Global.claimCount].Box11c.Value = Global.cfgFile['MedicareCarrierCode']
            else:
        ...
        def fld21_22():
            Global.Claims[Global.claimCount].Box21Ind.Value = '0' if (Global.cfgFile['ICD10']) else '9'
            tmpSep = '.' if (Global.cfgFile['FormVer'] == "02-12") else ' '  #08-05 form had pre-printed periods; 02-12 wants us to print them
            tmpStr = inLine[:10].translate(None, " .")  #Remove any pre-existing spaces or periods
            if (Global.cfgFile['ICD10']): tmpStr = self.gem.lookUp(tmpStr)
            Global.Claims[Global.claimCount].Box21_A.Value = tmpStr if len(tmpStr)<4 else (tmpStr[:3]+tmpSep+tmpStr[3:]) #Add period if more than 3 digits
            tmpStr = inLine[28:37].translate(None, " .")
            if (Global.cfgFile['ICD10']): tmpStr = self.gem.lookUp(tmpStr)
            Global.Claims[Global.claimCount].Box21_C.Value = tmpStr if len(tmpStr)<4 else (tmpStr[:3]+tmpSep+tmpStr[3:])
        ...
        def fld21_23():
            tmpSep = '.' if (Global.cfgFile['FormVer'] == "02-12") else ' '
            tmpStr = inLine[:10].translate(None, " .")
            if (Global.cfgFile['ICD10']): tmpStr = self.gem.lookUp(tmpStr)
            Global.Claims[Global.claimCount].Box21_B.Value = tmpStr if len(tmpStr)<4 else (tmpStr[:3]+tmpSep+tmpStr[3:])
            tmpStr = inLine[28:37].translate(None, " .")
            if (Global.cfgFile['ICD10']): tmpStr = self.gem.lookUp(tmpStr)
            Global.Claims[Global.claimCount].Box21_D.Value = tmpStr if len(tmpStr)<4 else (tmpStr[:3]+tmpSep+tmpStr[3:])

            #--- UltraMed occasionally outputs an empty diagnosis (and claim line pointers to it.)
            #--- Get rid of pointers to empty diags, shift valid diagnoses leftward
            #--- This will fail if UltraMed output a claim line with no valid diagnosis at all,
            #      but in that case we have bigger problems.
            tmpArray = ""
            diagList = []
            boxes = [   Global.Claims[Global.claimCount].Box21_A,
                        Global.Claims[Global.claimCount].Box21_B,
                        Global.Claims[Global.claimCount].Box21_C,
                        Global.Claims[Global.claimCount].Box21_D]
            for box in boxes:  #Gather valid diagnoses, build translation string for pointers
                if box.Value != "":
                    diagList.append(box.Value)
                    tmpArray += str(len(diagList))
                else:
                    tmpArray += " "
            Global.Claims[Global.claimCount].diagArray = tmpArray

            for i, box in enumerate(boxes):  #Re-fill form boxes with valid diagnoses
                try:
                    box.Value = diagList[i]
                except:
                    box.Value = ""
        ...
        def fld24():
            chkBlank  = string.maketrans("1234",Global.Claims[Global.claimCount].diagArray)
            ...
            if (len(inLine)>1 ): # adjust for (stupid extra blank lines) or (blank lines on last, total page)
                if fromTo2Digit.search(inLine) or fromTo4Digit.search(inLine) :   # this is the service line
                    ...
                    tmpStr = inLine[41:50].strip().translate(chkBlank)
                    tmpStr = tmpStr.translate(num2alpha)
        ...
        def fld25_30():
            ...
            Global.Claims[Global.claimCount].Box30.Value = string.rjust(inLine[70:80].strip(), 11)
        def bottom_1():
            Global.Claims[Global.claimCount].Box33Tel.Value         = inLine[62:].strip()
        def bottom_2():
            if Global.cfgFile['MediMedi']:
                Global.Claims[Global.claimCount].Box9a.Value        = inLine[22:49].strip()
            else:
        ...
        with open(thisFile,'r+b') as inFile:
            for tmpLine in inFile.readlines():
                ...
                try:
                    ...
                    tmpStr += "\nError:\n[%s]" % inst
                    print(logging.exception(tmpStr))
                    raise
                Global.lineCount +=1
    ...
    def GetOnWithIt(self, pathStr, target=None, TestPrint=False):
            ...
            if len(Global.npiMap.noMatch) > 0:
                ...
                if not Global.cfgFile['IncludeLegacy']:
                    ...
                    if (response == wx.ID_NO):
                        self.Close(True)
                        sys.exit()
            if len(self.gem.Notes) > 0:
                tmpStr = "Your code:  Conversion status\n\n"
                for code, notes in self.gem.Notes.iteritems():
                    tmpStr += '{:9}:  {}\n'.format(code, notes)
                tmpStr += "\nProceed?"
                dlg = wx.MessageDialog(None, tmpStr,
        ...
        if self.radio3.GetValue(): # Printing
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                wdth, hght = (4850, 6600)  #because GetSize lost its damn mind in Windows 8
                #print "wdth: {}  hght:  {}".format(wdth,hght)
...
class MyApp(wx.App):
    def OnInit(self):
        frame = MyFrame(None, -1, "FSR-1500",wx.Size(550,450))
        frame.Show()
        return True
