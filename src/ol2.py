...
class MSOutlook(object):
    def __init__(self):
        try:
            self.outlookApp = gencache.EnsureDispatch("Outlook.Application")
            self.outlookFound = True
        except:
            print "MSOutlook: unable to load Outlook"
            self.outlookFound = False
        self.records = [  ]
    def loadMessages(self, keys=None):
        if not self.outlookFound: return
        onMAPI = self.outlookApp.GetNamespace("MAPI")
        inBox = onMAPI.GetDefaultFolder(constants.olFolderInbox)
        if DEBUG: print "number of messages:", len(inBox.Items)
...
#        for msg in range(len(inBox.Messages)):
#            contact = inBox.Items.Item(oc + 1)
