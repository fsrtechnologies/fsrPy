...
def fsrRSP():
    ...
    """
    logger.info("cfgFileName: " + cfgFileName)
    ...
    for ediDir in cfgFile['ediDirs']:
        ...
        for inObj in os.listdir(ediDir):
            #first pass, extract any zip files to current dir and move them (zips) to Old
            inFile = ediDir + os.sep + inObj
            logger.info(inFile)
            if zipfile.is_zipfile(inFile):
                logger.info(inFile + 'is a zipfile')
                ...
                while attempt > 0:  #avoid blowing up due to duplicate filename
                    try:
                        ...
                        dst_file = os.path.join(ediOld, '%s(%s).zip' % (os.path.splitext(basename)[0], attempt))
                        attempt += 1
        ...
        for inObj in os.listdir(ediDir):
            ...
            logger.info("Processing %s ..." % ediFile)
            try:
                ...
                if tail == ".TRNACK":
                    ...
                    logger.warn("%s is not an X12 file" % ediFile)
                    continue
            if thingy.TA1:
                ...
                for gsLoop in thingy.GSLoop:
                    ...
                    for stLoop in gsLoop.STLoop:
                        if stLoop.STRec[1] == '277':
                            ...
                            logger.info(stLoop.STRec)
                        try:
                            thisX12.toPDF()
                        except NameError:
                            logger.error("Error processing %s as an X12 file" % ediFile)
            count = 0
