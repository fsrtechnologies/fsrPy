...
class Global(object):
    ...
    cfgFileName = os.getcwd() + os.sep + 'pat_exp.ini'
    tmpStr = """
...
class Log(object):
    ...
    def __init__(self):
        with open(os.getcwd() + os.sep + 'pat_exp.log', 'w') as logfile:
    ...
    def write(self,text_string, fileOnly=False):
        with open(os.getcwd() + os.sep + 'pat_exp.log', 'a+') as logfile:
...
class Selector(wx.Frame):
    def __init__(self, parent, id, title, size=(750,1250)):
        ...
        (Ignore the year - it defaults to 2012 because it was a leap year, so you can select Feb 29.)
        ...
        self.BDayStart = wx.DatePickerCtrl(leftPanel, dt=(wx.DateTimeFromDMY(1,0,2012)), size=(120,-1), style=wx.DP_DROPDOWN | wx.DP_SHOWCENTURY | wx.DP_ALLOWNONE)
        self.BDayEnd   = wx.DatePickerCtrl(leftPanel, size=(120,-1), style=wx.DP_DROPDOWN | wx.DP_SHOWCENTURY | wx.DP_ALLOWNONE)
        self.BDayStart.SetRange(wx.DateTimeFromDMY(1,0,2012), wx.DateTimeFromDMY(31,11,2012))
        self.BDayEnd.SetRange(wx.DateTimeFromDMY(1,0,2012), wx.DateTimeFromDMY(31,11,2012))
        ...
        If you leave both dates un-checked, we'll export patients whose PRIMARY doctor is one of the ones you've selected.'''
        ...
        Should we skip Address 1 if it looks like a phone number?'''
        ...
        tipString = '''Should we export patients from all ZIP codes, or only for a range of ?'''
    ...
    def OnBtnLabels(self, event):
        ...
        if not bDayStart.IsValid(): bDayStart = wx.DateTimeFromDMY(1,0,2012) #using a leap year to catch any
        if not bDayEnd.IsValid(): bDayEnd = wx.DateTimeFromDMY(31,11,2012)   # Feb 29 birthdays
        ...
        if self.cbSepBad.IsChecked():
            BadFile = csv.writer(open(Global.d_top + os.sep + 'BadPats.csv', 'wb'), quoting=csv.QUOTE_ALL)
            BadFile.writerow(['First Name', 'Middle Name', 'Last Name', 'Employer', 'Office Name', 'Office Phone', 'Office Phone Ext.', 'Office Fax', 'Home Phone', 'Social Security', 'Marital Status', 'Gender', 'Date of Birth', 'Address Line 1', 'Address Line 2', 'City', 'State', 'Zip Code', 'Comments'])
